...
class CourseListManager(PaginateMixin):
    @inject
    def __init__(
        self, service: CourseApiService, channel_service: ChannelPublicApiService
    ):
    ...
    def _get_item_command(self, item: Any, **kwargs):
        return f"/init_show_course {str(item.id)}"

    def _get_list_header(self, context: CallbackContext, **kwargs):
        channel: Channel = self.channel_service.get_by_id(
            context.user_data["course_channel"]["channel_id"], context.user
        )
        return _("channel_curses_header") % channel.name

    def _callback_subfix(self, **kwargs):
        return "channel_courses"
    ...
    def callback(self, update: Update, context: CallbackContext):
        ...
        context.user_data["course_channel"] = {"channel_id": params[0]}
    ...
    def _get_list_data(self, update: Update, context: CallbackContext, **kwargs):
        ...
        user_data = context.user_data["course_channel"]
        params = {"channel": user_data["channel_id"]}
        data = service.list(user=context.user, **params, **kwargs)
        context.user_data["course_channel"] = {**user_data, **{"count": data["count"]}}
        return data

    def _list_out(self, context: CallbackContext, **kwargs):
        return f'/channel_detail {context.user_data["course_channel"]["channel_id"]}'
